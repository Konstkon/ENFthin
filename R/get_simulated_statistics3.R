...
#'
#'
#' @title Overall centre three-dimenstional L curves from patterns in a list
...
#' @param{point_pattern_list} A list of size [N] [M] containing three dimensional point patterns ("pp3" objects).
#' @param{nsim}  Number of simulations. Default is nsim=2500.
#' @param{RMAX} Maximum value for the radius parameter
#' @return A matrix where each column is a pooled estimate of the three dimensional centred L function
#' from patterns that are obtained at random from the list.
#' @export
#'
#' @examples
#' get.simulated_statistics3(simulations)
#'
get.simulated_statistics3<-function( point_pattern_list,
                                     nsim=2500,
                                     RMAX=30){
  #Get random indices
  ...
  for( i in 1:nsim){
    ...
    simulated = Overall_centred_L3est(simulated_samples,RMAX = RMAX)
    if(i==1){
