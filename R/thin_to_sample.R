...
thin_to_sample <-function(ppp_ep,
                          ppp_bp ,
                          num_ep ,
                          num_bp ,
                          epsilon = 0,
  ...
  while(num_points_ppp_bp  > (num_bp + epsilon) ){
    ...
    if(scheme == "random"){
      #ASSIGN RANDOM MARKS
      thinned_ppp_bp = assign_id_to_pattern(thinned_ppp_bp)
    ...
    if(scheme == "dependent"){
      ...
      thinned_ppp_bp$marks$PROBS =PROBS
      #REMOVE ONE POINT BASED ON MARKS
      thinned_ppp_bp = thin_with_probability(thinned_ppp_bp)
    }
